...
- 播放器控制层已补齐上一集、播放/暂停、下一集、进度和画中画相关控制；画中画只能由用户点击播放器按钮触发，普通 Home/最近任务退后台必须暂停且禁止自动弹出小窗。
- 画中画只渲染视频画面，系统 RemoteAction 提供上一集、播放/暂停、下一集；从小窗展开时必须恢复进入前的竖屏或显式全屏状态。
- 系统自然横屏使用播放器 2/3 + 信息/来源/选集 1/3；只有用户主动点击全屏按钮时隐藏侧栏并占满窗口，画中画不得复用双栏，退出全屏不得强制锁回竖屏。
- 从播放器相关推荐继续打开作品时必须压入播放页快照；系统侧滑、返回键和播放器返回按钮都应逐级恢复上一作品，不能把返回目标写成 `player` 自己。
...
- 首页季度新番榜、首页季度排期、底部周表和底部排行榜是四个不同页面：周表只表示本周更新；排行榜只区分日漫TV番剧与国漫动画（剧场版分类已移除）。禁止再把季度页签塞回底部周表或通用排行榜。
- 卡片模型可以保留 `MediaStatus.UNKNOWN`，但前端不得展示“状态待确认”；显示层只能回退到明确集数、来源状态、年份或中性的“已收录”，不得把未知状态猜成连载或完结。
- 播放详情必须使用请求代次隔离；迟到请求不得覆盖当前作品、剧集或来源列表。同源 ID 返回空剧集时必须按规范化标题恢复真实 ID，且只有非空详情可写入详情缓存。
- 收藏按钮必须观察 `favoritesList` 快照状态，不能只在 Composable 中读取 `SharedPreferences`；历史记录保存进度但继续禁止持久化临时/签名播放 URL。
- 播放器一次拖动手势只能锁定为纵向亮度/音量或横向进度之一。GPU画质增强只能陈述实际锐化/色彩能力；未接入厂商NPU模型时禁止标注“AI 4K超分”。
