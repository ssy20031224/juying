...
## 2026-08-08：AuvFun 来源状态对齐与双星协议修复

- 为 Lanerc、番薯、三秋、薯条建立统一可用性状态；首页切源、设置数据源测试、诊断测试和片库筛选在请求前直接展示“源维护中/源异常”，不再把已知停用源误报成普通网络失败。
- 启动默认选择首个“启用、可用且适配器加载成功”的来源，而不是默认“全部源”；首页单源模式同步切换轮播、首页分区、季度新番榜、季度排期、周表、排行榜及日漫/国漫/剧场版分类。
- 来源没有专用榜单/排期接口时，只用当前来源首页分区和明确分类字段生成回退内容，不跨源补齐，也不生成虚构热度、评分或更新时间。
- 修正单源首页的分区数据引用，改为使用本次切源实际加载的 `homeSourceSections`。
- 播放失败按“当前集下一线路 → 其他来源”的顺序自动恢复，两个阶段分别受“自动切线路”和“自动换源”设置控制。
- 播放器设置实装画面裁剪、全局片头/片尾秒数、后台播放、外挂字幕、定时关闭；倍速、长按倍速和裁剪选项会与全局设置同步。
- 清晰度默认显示 `Auto`：AuvFun、金牌可使用脚本返回的真实多清晰度地址，自适应 HLS 可使用 Media3 发现的真实视频轨；普通单地址 MP4 不再伪造清晰度选项。
- 逆向确认 AuvFun 使用 ExoPlayer/MPV 双内核；聚映当前只集成 Media3 ExoPlayer，因此“自动换内核”明确标为未启用，不再提供虚假的 Ijk/System 选项。
- 咕咕播放链路在线复核确认 `vodParse` 当前返回“点数不足”；脚本与播放页会透传服务端原因，不再统一显示“解析超时”。

- 核对 AuvFun 在线配置 `version=8`（`updatedAt=2026-08-08T00:12:10Z`）：其“11 个正常源”是 14 个登记源中排除了已关闭的 Lanerc、番薯、薯条，并不是 11 个来源都完成了详情和正片播放验证。
- Android 默认关闭上游已关闭的 Lanerc、番薯、薯条；额外的三秋源因不在当前 AuvFun 清单且唯一已知 API 主机返回 403，也改为默认关闭。用户仍可在来源设置中手动开启。
- 修复双星宿主注入的接口根路径缺少 `/app/bn`，并把签名时间从宿主毫秒值换算为 App99 协议要求的 Unix 秒。
- 番薯兼容脚本可识别服务端轮换请求头时返回的 `REQUEST_HEADER_INVALID`；当前上游仍将该源关闭，不把首页连通误报为可播放。
- 播放器仍通过 Media3 `OkHttpDataSource` 将来源返回的 Referer、User-Agent 和自定义 Header 透传到 HLS 主清单、密钥与分片；本次故障发生在来源详情/播放解析或上游 CDN，未发现 ExoPlayer 丢头。
- 下载设置说明已与真实行为对齐：Wi-Fi 限制只在新任务启动时检查；MP4 支持 Range 时并行分段，但当前失败任务会重新下载，不宣称断点续传。
