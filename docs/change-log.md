...
## 2026-08-06：下载/点赞图标、历史分组细化、评论定位跳转与分享修复

- 新增 `ic_download`（下载）与 `ic_like`（点赞）SVG 图标：播放器"下载番剧"按钮、个人页"下载记录"入口改用下载图标；评论区点赞由 👍 表情改为点赞图标（已赞显示玫红色并保留数量）。
- 观看历史与追番分组细化为"今天 / 昨天 / 前天 / 近一周 / 更早"（原为近一周/近一月/近半年/更早），`historyPeriodLabel` 边界同步更新并更新对应单测。
- 子评论（回复）新增时间显示（与主评论同一时间格式）。
- 消息通知"评论回复"提醒点击后：打开对应作品并自动切换到评论页签、滚动定位到该评论并高亮 4 秒；"追番更新"提醒仍跳转对应番剧。
- 播放器"分享"按钮修复：不再禁用，点击唤起系统分享面板（ACTION_SEND），分享内容包含作品名、当前剧集和播放地址。
- 验证：Android `:app:compileDebugKotlin`、`:app:testDebugUnitTest`（34/34）、`:app:assembleDebug` 通过。

## 2026-08-05：SVG 图标替换、首页分类内容切换与历史/追番管理删除

- 首页通知铃铛、观看历史时钟、播放器换源、头像相册/拍照、底部排行榜导航全部替换为 SVG 矢量图标（`ic_notification`、`ic_history`、`ic_switch_source`、`ic_album`、`ic_camera`、`ic_ranking`）；评论区图片上传按钮同步改用 `ic_album`；`ActionButton` 增加 Painter 重载以支持矢量图标。
- 首页“精选 | 日漫 | 国漫 | 剧场版”页签切换完整内容：分类页签内嵌展示仅当前分类的轮播图（角标显示分类名）、3x3 热门推荐网格（9 部）、推荐栏右侧“查看更多”进入对应分类片库；轮播与热门推荐每 10 秒轮换一批内容。
- 分类聚合采用严格互斥分类器（日漫/国漫/剧场版标题+分类+标签三重证据），日漫与国漫互不混入，剧场版/电影作品只归入剧场版分类。
- 观看历史与我的追番删除交互重构：移除卡片左滑删除与“清空记录”，右上角改为“管理”；点击后卡片可单独勾选或底部栏“选择全部”全选，右下角“删除”批量移除；“管理”变为“完成”，点击执行所选删除后退出管理模式。追番页移除每卡片“取消收藏”按钮，改用同样的管理模式。
- 验证：Android `:app:compileDebugKotlin`、`:app:testDebugUnitTest`、`:app:assembleDebug` 通过。
