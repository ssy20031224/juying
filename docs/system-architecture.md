...
- Android 通过 `LanercDiscoveryRepository` 原生移植已审计脚本的数据契约，但不下载或执行远程 JS。榜单和排期按北京时间只展示当前年份的当前季度与同年上一季度；未来季度和上一年份季度不会混入。
...
- Android 亮度/音量拖动与长按倍速互斥；锁屏或非画中画退后台会终止临时手势、恢复用户倍速并暂停，剧集数据由 ViewModel 保留并重新传入播放器。
