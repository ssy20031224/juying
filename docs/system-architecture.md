...
- Android 使用统一 `MediaStatusSummary` 解析来源备注，只接受明确标记判断完结、连载、更新和未开播；裸集数与空值保持“状态待确认”，详情页的实际剧集列表只用于展示集数，不反向猜测连载状态。
...
- Android 通过 `LanercDiscoveryRepository` 原生移植已审计脚本的数据契约，但不下载或执行远程 JS。热门/人气保留后端季度组顺序，评分榜跨组按真实数值评分排序；不生成虚构热度、播放量或评分。
