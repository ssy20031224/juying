...
> 文档状态：2026-08-08（Android 来源状态与播放链路复核）
> 适用提交：当前工作区最新提交  
...
- 更新检查并行比较多份 `update.json` 的版本与修订号，同版本优先发布者配置 URL。当前来源 JS 以 APK 内置审计副本为权威版本；`LANERC_SOURCE_SCRIPT_BASE_URL`/旧 CDN 只在内置副本缺失或损坏时回退，启动同步会清理历史远程缓存，尚未恢复带签名清单的热更新。
- AuvFun 在线配置在 2026-08-08 登记 14 源并关闭 Lanerc、番薯、薯条，因此其诊断页显示 11 个启用源。Android 另登记三秋，但该源当前 403，以上四源均默认关闭；来源首页诊断只表示列表连通，不等价于详情、解析和正片内容可用。
- Android 使用 `SourceAvailability` 区分可用、维护中和异常来源。已知不可用来源不会初始化适配器或发起片库/播放请求，但仍在来源管理和首页切源菜单中展示状态与原因。
- Android 冷启动默认选择首个启用、可用且适配器加载成功的来源；“全部源”保留为用户主动选择的聚合模式。选择单一来源后，轮播、首页分区、季度榜/排期、周表、排行榜及日漫/国漫/剧场版均从同一来源快照派生；缺少专用接口时只按该来源的分区标题与明确分类字段回退，不跨源补齐。
...
- 清晰度以真实能力为准：来源脚本返回 `resolutions` 时显示对应地址，自适应 HLS 由 Media3 轨道信息生成可选分辨率，普通单地址 MP4 只显示 `Auto`。AuvFun 逆向样本包含 ExoPlayer/MPV 双内核，Android 当前未引入 MPV，因此自动换内核保持关闭并在 UI 标注未启用。
- 播放设置由 `StorageManager` 持久化并在全局设置页与播放器内设置同步，包括默认倍速、长按倍速、画面裁剪、片头/片尾、后台播放和自动恢复策略；外挂字幕和定时关闭属于当前播放会话。后台播放当前依赖进程内播放器生命周期，尚未升级为带媒体通知的 `MediaSessionService`。
