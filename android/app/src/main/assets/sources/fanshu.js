...
var FANSHU_GUARD_HEADER = 'X-App-Guard-543115e6';
...
function _headers(signature, secretRequest) {
    ...
    h[FANSHU_GUARD_HEADER] = FANSHU_GUARD;
    if (secretRequest) h['X-API-TOKEN'] = FANSHU_TOKEN;
    return h;
}

function _missingGuardHeader(body) {
    try {
        var j = _parse(body);
        if (j && (j.error_code === 'REQUEST_HEADER_MISSING' ||
                  j.error_code === 'REQUEST_HEADER_INVALID') && j.header &&
            /^X-App-Guard-[0-9a-f]+$/i.test(String(j.header))) {
        ...
        if (typeof http !== 'undefined' && http) {
            try {
                ...
                if (r && typeof r === 'object') {
                    var res = { status: Number(r.status || 0), body: String(r.body == null ? '' : r.body), ok: r.ok !== false };
                    if (res.status === 403 && guardRetry < 1) {
                        var need = _missingGuardHeader(res.body);
                        if (need && headers[need] == null) {
                            var old = FANSHU_GUARD_HEADER;
                            FANSHU_GUARD_HEADER = need;
                            headers[need] = FANSHU_GUARD;
                            if (old !== need) { try { delete headers[old]; } catch (e) {} }
                            guardRetry++;
                            continue;
                        }
                    }
